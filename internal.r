...
#fromto=c(from, to)
sampler_regPassive <- function(p, data, m, imp, r, visitSequence, fromto, printFlag, ...)
            ...
            for (i in 1:m) {
                ...
                for (j in p$visitSequence) {
                  ...
                  if (theMethod != "" & (!is.passive(theMethod)) & theMethod != "dummy") {
                    ...
                    #r <- (!is.na(p$data)) # FALSE if missing
                    #for (j in p$visitSequence)
                    #for (i in 1:m)
                    #p$data[!r[, j] # missing data
                    
                    #m_ps <- glm(treatment ~ X1 + X2, data=p$data, family=binomial)
                    #imp[[j]][, i] = predict(m_ps, type = "response")[!r[, j]]
                    imp[[j]][, i] = glm(treatment ~ X1 + X2, data=p$data[!r[, j], ], family=binomial)$fitted
                    p$data[!r[, j], j] <- imp[[j]][, i]
