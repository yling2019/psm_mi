...
for (i in 1:nSimulation){
	...
	if (b==nSubj) {
		r = foreach( i = 1:nSubsample, .combine=rbind ) %dopar% {
			...
			MIwithPSmatching(complete.data=cd, simulated.data=id, impMethod=impMethod, intMethod=intMethod, MIwithAux=MIwithAux, missingVariables=missingVariables, confounders=confounders, nCovariates=nBin+nNor, outcomeBetas=0, nBin=nBin, nNor=nNor, caliper=caliper, m=m)
		}
	} else {
		r = foreach( i = 1:nSubsample, .combine=rbind ) %dopar% {
