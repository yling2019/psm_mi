...
mice_regPassive <- function(data, m = 5, method = vector("character", length = ncol(data)), predictorMatrix = (1 - diag(1, ncol(data))),
    ...
    defaultImputationMethod = NULL, data.init = NULL, ...)
{
    ...
    from <- 1
    to <- from + maxit - 1
    q <- sampler_regPassive(p, data, m, imp, r, visitSequence, c(from, to), printFlag, remove_collinear=FALSE)
